...
export default function StreakGrid({ currentDay, submissions, questions, tiltProps = {} }) {
  ...
  return (
    <div className="streak-grid-wrapper streak-grid-compact press-card" {...tiltProps}>
      ...
      {(!activeDayInfo || activeDayInfo.day <= currentDay) && (
        <div className="previous-day-section" aria-live="polite">
          {activeDayInfo ? (
            activeDayInfo.day % 7 === 0 ? (
              <div className="grid-tooltip-compact grid-tooltip-static">
                <div className="tooltip-header">
                  <span className="tooltip-day-title">Day {activeDayInfo.day} · Sunday Debug Special</span>
                  <span className={`tooltip-status-badge ${activeDayInfo.status.class}`}>
                    {activeDayInfo.status.text}
                  </span>
                </div>
                <div className="tooltip-body">
                  {activeDayInfo.debugChallenge ? (
                    <>
                      <h4 className="previous-day-title" style={{ margin: '0.5rem 0 0.25rem 0', fontSize: '0.95rem', fontWeight: 'bold' }}>
                        Theme: {activeDayInfo.debugChallenge.theme}
                      </h4>
                    </>
                  ) : (
                    <p className="tooltip-q-item">No debugging challenge scheduled/released yet.</p>
                  )}
                </div>
              </div>
            ) : (
